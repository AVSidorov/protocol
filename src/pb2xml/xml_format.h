...
class XML_UTIL_EXPORT XmlFormat {
 public:
  ...
  static void PrintToXmlString(const Message& message, std::string* output);
  ...
  class XML_UTIL_EXPORT Printer {
   public:
    ...
    void PrintToXmlString(const Message& message, std::string* output);
   ...
   private:
    ...
    // on field type
    const std::string & GetXmlFieldName(const Message& message,
                        const Reflection* reflection,
                        const FieldDescriptor* field);
    ...
    // Hold new allocated object pointers for later deleting at destructor function,
